/**
 * REMOVAL-9-MONTH-BACKFILL-DOMAIN-SYNC-CHUNK1 — staging only
 *
 *   npx tsx scripts/removal-9-month-backfill-domain-sync-chunk1-execute.ts --apply
 */
...
const OUT_BASE = ".cursor/audit-reports/removal-9-month-backfill-domain-sync-chunk1";

const CHUNK_WINDOW = {
  start: "2025-08-27T00:00:00.000Z",
  end: "2025-08-31T23:59:59.999Z",
};
const ORDER_UPLOAD_ID = "924ce268-8840-497b-8da4-c9cb7879afbf";
const SHIPMENT_UPLOAD_ID = "0911b550-b87a-4137-aa48-dd0093c5645b";
...
async function main(): Promise<void> {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      `| chunk window | ${CHUNK_WINDOW.start} → ${CHUNK_WINDOW.end} |`,
  ...
  fs.writeFileSync(
    path.join(outDir, "chunk1-upload-ids.json"),
    JSON.stringify(
      {
        window: CHUNK_WINDOW,
        expected_rows: { order: 10, shipment: 25 },
        uploads: [
  ...
  if (!apply || blockers.length) {
    ...
    fs.writeFileSync(path.join(outDir, "manifest.json"), JSON.stringify({ run_id: runId, ok: !blockers.length, apply: false, blockers }, null, 2));
  ...
  fs.writeFileSync(
    ...
    [
      "# Domain sync + rebuild summary",
      "",
      "| Step | Result |",
      "|------|--------|",
      `| REMOVAL_ORDER pipeline | ${orderPipe.ok ? "PASS" : "FAIL"} (\`${orderPipe.state}\`) |`,
      `| REMOVAL_SHIPMENT pipeline | ${shipPipe.ok ? "PASS" : "FAIL"} (\`${shipPipe.state}\`) |`,
      `| rebuild_expected_packages_from_removals | detail_lines=${rebuild.detail_lines_in_scope} matched=${rebuild.matched_rows_upserted} remainder=${rebuild.remainder_rows_upserted} |`,
      `| resolver rows scanned | ${resolverStats.scanned} |`,
      `| resolver rows applied | ${resolverStats.applied} |`,
    ].join("\n") + "\n",
  );
  ...
  fs.writeFileSync(
    ...
    [
      "# Code change summary",
      "",
      "Execute-only run — no application code changes in this pass.",
      "",
      "| Script | Role |",
      "|--------|------|",
      "| `scripts/removal-9-month-backfill-domain-sync-chunk1-execute.ts` | This orchestrator |",
      "| `lib/amazon/reports-api-pipeline-handoff.ts` | Domain sync pipeline |",
      "| `rebuild_expected_packages_from_removals` | EP rebuild |",
      "| `lib/removal/resolve-expected-package-product.ts` | Resolver backfill |",
    ].join("\n") + "\n",
  );
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `| resolved (cohort) | ${beforeEp.resolved_total} | ${afterEp.resolved_total} | ${afterEp.resolved_total - beforeEp.resolved_total} |`,
      "",
      `rebuild_valid: **${rebuildValid ? "yes" : "no"}** (mismatch=${mismatchCount})`,
    ].join("\n") + "\n",
  );
    ...
    JSON.stringify(
      {
        ...
        domain: {
          ...
          chunk_order_rows: orderAfter - orderBefore,
          chunk_shipment_rows: shipAfter - shipBefore,
        },
  ...
  fs.writeFileSync(
    ...
    [
      `-- Rollback chunk1 domain sync run_id=${runId}`,
  ...
  const nextPrompt =
    ok
      ? "REMOVAL-9-MONTH-BACKFILL-FETCH-CHUNK2 — fetch next missing window, then REMOVAL-9-MONTH-BACKFILL-DOMAIN-SYNC-CHUNK2"
      : "REMOVAL-9-MONTH-BACKFILL-DOMAIN-SYNC-CHUNK1 — fix blockers and re-run";
