/**
 * PC05D-WAVE1-ACTIVATE — Activate eligible Wave 1 versions on staging (approval-gated).
 *
 *   npx tsx scripts/pc05d-wave1-activate-staging.ts --apply
 */
...
const EXECUTE_RUN = "20260526T190000Z";
const REVIEW_RUN = "20260526T192000Z";
const BATCH_TAG = `PC05D_WAVE1_${EXECUTE_RUN}`;
const APPROVAL_PATH = ".cursor/operator-approvals/product-packaging-backfill-pc05d-scale-activate-approval.md";
const EXECUTE_DIR = `.cursor/audit-reports/pc05d-wave1-packaging-scale-staging-execute/${EXECUTE_RUN}`;
const REVIEW_DIR = `.cursor/audit-reports/pc05d-wave1-review-census-staging/${REVIEW_RUN}`;
const OUT_BASE = ".cursor/audit-reports/pc05d-wave1-activate-staging";
...
function readApprovalFlags(): { run: boolean; activate: boolean; raw: Record<string, string> } {
  ...
  const actM =
    text.match(/APPROVED_PRODUCT_PACKAGING_BACKFILL_PC05D_SCALE_ACTIVATE\s*=\s*(\S+)/) ??
  ...
  return {
    ...
    raw: {
      ...
      APPROVED_PRODUCT_PACKAGING_BACKFILL_PC05D_SCALE_ACTIVATE: actVal,
    },
  };
}
...
async function main(): Promise<void> {
  ...
  const eligibleVersionIds = fs
    ...
    .map((l) => l.trim())
    .filter(Boolean);
  if (eligibleVersionIds.length !== 50) {
    blockers.push(`Expected 50 eligible version ids, got ${eligibleVersionIds.length}`);
  }
  ...
  const eligibleSet = new Set(eligibleVersionIds);
  for (const vid of eligibleVersionIds) {
    if (!allWaveVersionIds.has(vid)) blockers.push(`Eligible version not in wave1 execute: ${vid}`);
  }
  ...
  fs.writeFileSync(
    ...
    [
      "# Approval proof — PC05D Wave 1 activate",
      "",
      ...
      `APPROVED_PRODUCT_PACKAGING_BACKFILL_PC05D_SCALE_ACTIVATE=${approval.raw.APPROVED_PRODUCT_PACKAGING_BACKFILL_PC05D_SCALE_ACTIVATE}`,
      "```",
      "",
  ...
  if (blockers.length === 0 && dbUrl) {
    ...
    postImage = { table_counts: countsAfter };
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ ...preImage, pre_targets: preTargets }, null, 2),
  );

  fs.writeFileSync(
    path.join(outDir, "activation-result.md"),
    [
      "# Activation result — PC05D Wave 1",
      "",
      `Run: \`${runId}\``,
      `Apply: **${apply ? "YES" : "NO"}**`,
      "",
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      `Expected dimensions_current delta: **+50** (191 → 241)`,
    ].join("\n"),
  );
  ...
  fs.writeFileSync(
    ...
    [
      "-- PC05D Wave 1 activate rollback — revert to needs_review",
      `-- Activate run: ${runId}`,
      `-- Batch: ${BATCH_TAG}`,
      "",
      "BEGIN;",
  ...
  const ok =
    ...
    activated === 50 &&
    snapshotPass === 50 &&
    (postImage.table_counts as Record<string, number>)?.dimensions_current === 241;

  fs.writeFileSync(
    path.join(outDir, "manifest.json"),
    JSON.stringify(
      {
        prompt: "PC05D-WAVE1-ACTIVATE — PACKAGING SCALE WAVE 1 REVIEW ACTIVATE STAGING",
        run_id: runId,
