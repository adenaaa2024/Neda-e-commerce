/**
 * PC05E-ORIGINAL-VERIFY — Wave 1 packaging original parity census (read-only).
 *
 *   npx tsx scripts/pc05e-wave1-original-verify-census.ts
 *   npx tsx scripts/pc05e-wave1-original-verify-census.ts --execute-run-id=20260526T200000Z
 */
...
const PC05E_EXECUTE_DEFAULT = "20260526T200000Z";
const BATCH_TAG = `PC05D_WAVE1_${WAVE1_EXECUTE_RUN}`;
const OUT_BASE = ".cursor/audit-reports/pc05e-wave1-original-verify";
const EXPECTED_WAVE1 = 50;
const EXPECTED_ORIGINAL_CURRENT = 241;
...
function executeRunIdArg(): string {
  ...
  return a ? a.split("=")[1]!.trim() : PC05E_EXECUTE_DEFAULT;
}
...
async function main(): Promise<void> {
  ...
  const executeManifestPath = path.join(
    process.cwd(),
    ".cursor/audit-reports/pc05e-wave1-original-parity-execute",
    executeRunId,
    "manifest.json",
  );
  ...
  if (!fs.existsSync(executeManifestPath)) {
    blockers.push(`Missing PC05E execute proof: ${executeRunId}`);
  } else {
    try {
      ...
      if (!em.ok || (em.inserted_profiles ?? 0) < EXPECTED_WAVE1) {
        blockers.push(`PC05E execute not ok or <${EXPECTED_WAVE1} inserts`);
      }
    } catch {
      blockers.push("Invalid PC05E execute manifest");
    }
  }

  if (fs.existsSync(activateManifestPath)) {
    try {
      const am = JSON.parse(fs.readFileSync(activateManifestPath, "utf8")) as { ok?: boolean };
      if (!am.ok) blockers.push("Wave1 activate manifest not ok");
    } catch {
  ...
  if (blockers.length === 0) {
    ...
    if (stagingWave1.size !== EXPECTED_WAVE1) {
      blockers.push(`Expected ${EXPECTED_WAVE1} activated Wave1 rows on staging, got ${stagingWave1.size}`);
    }
  ...
  const counts = {
    staging_active_wave1: stagingWave1.size,
  ...
  const parityPass =
    counts.staging_active_wave1 === EXPECTED_WAVE1 &&
    counts.original_matched === EXPECTED_WAVE1 &&
  ...
  fs.writeFileSync(
    path.join(outDir, "wave1-parity-census.md"),
    [
      "# Wave 1 parity census — PC05E-ORIGINAL-VERIFY",
      "",
      `**Run id:** \`${runId}\``,
      `**Branch:** \`${branch}\``,
      ...
      "|-------|----------|--------|------|",
      `| staging_active_wave1 | ${EXPECTED_WAVE1} | ${counts.staging_active_wave1} | ${counts.staging_active_wave1 === EXPECTED_WAVE1 ? "YES" : "NO"} |`,
      `| original_matched | ${EXPECTED_WAVE1} | ${counts.original_matched} | ${counts.original_matched === EXPECTED_WAVE1 ? "YES" : "NO"} |`,
      ...
      "",
      "Pilot 191 + Wave 1 50 = 241 expected on `dimensions_current`.",
    ].join("\n"),
  );
    ...
    JSON.stringify(
      {
        ...
        wave1_batch_tag: BATCH_TAG,
        pc05e_execute_run: executeRunId,
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `- Script: \`scripts/pc05e-wave1-original-verify-census.ts\``,
    ].join("\n"),
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        prompt: "PC05E-ORIGINAL-VERIFY — WAVE 1 PACKAGING ORIGINAL PARITY CENSUS",
        run_id: runId,
        branch,
        staging_ref: STAGING_REF,
        original_ref: ORIGINAL_REF,
