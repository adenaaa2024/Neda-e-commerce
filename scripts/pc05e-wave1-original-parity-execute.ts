/**
 * PC05E-ORIGINAL-EXECUTE — Apply Wave 1 packaging parity to original/current (approval-gated).
 *
 *   npx tsx scripts/pc05e-wave1-original-parity-execute.ts --apply
 *   npx tsx scripts/pc05e-wave1-original-parity-execute.ts --apply --plan-run-id=20260526T194000Z
 */
...
const PLAN_DEFAULT = "20260526T194000Z";
const APPROVAL_PATH = ".cursor/operator-approvals/product-packaging-wave1-original-parity-pc05e-approval.md";
const PLAN_BASE = ".cursor/audit-reports/pc05e-wave1-original-parity-plan";
const OUT_BASE = ".cursor/audit-reports/pc05e-wave1-original-parity-execute";
const EXPECTED_ROWS = 50;
const EXPECTED_CURRENT_AFTER = 241;
...
function readApprovalFlags(): { run: boolean; parity: boolean; raw: Record<string, string> } {
  ...
  const parM = text.match(/APPROVED_PRODUCT_PACKAGING_WAVE1_ORIGINAL_PARITY\s*=\s*(\S+)/);
  const runVal = runM?.[1] ?? "";
  const parVal = parM?.[1] ?? "";
  return {
    run: runVal === "true",
    ...
    raw: {
      ...
      APPROVED_PRODUCT_PACKAGING_WAVE1_ORIGINAL_PARITY: parVal,
    },
  };
}
  ...
  for (const row of sample.length ? sample : insertedIds.slice(0, 1)) {
    ...
    const ver = await client.query(
      `SELECT profile_status, evidence_summary->>'pc05_candidate_id' AS cid,
              evidence_summary->>'pc05e_original_execute_run_id' AS exec_run
    ...
    if ((ver.rowCount ?? 0) !== 1) failures.push(`${row.candidate_id}: version missing`);
      ...
      if (ver.rows[0].exec_run !== executeRunId) failures.push(`${row.candidate_id}: missing pc05e execute run id`);
    }
...
async function main(): Promise<void> {
  ...
  const batchTag = `PC05E_WAVE1_ORIGINAL_${runId}`;
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `APPROVED_PRODUCT_PACKAGING_WAVE1_ORIGINAL_PARITY=${approval.raw.APPROVED_PRODUCT_PACKAGING_WAVE1_ORIGINAL_PARITY}`,
      "```",
  ...
  fs.writeFileSync(
    ...
    [
      "-- PC05E Wave 1 original parity rollback",
    ...
    if (hardBlockers.length === 0) {
      ...
      for (const row of insertPlan) {
        ...
        const evidence = {
          ...
          pc05e_original_execute_run_id: runId,
          pc05e_wave1_original_batch_tag: batchTag,
  ...
  fs.writeFileSync(
    ...
    [
      "# Execute result — PC05E Wave 1 original",
      "",
      `Apply: **${apply ? "YES" : "NO"}**`,
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      `Expected current after: **${EXPECTED_CURRENT_AFTER}** (191 pilot + 50 wave1)`,
    ].join("\n"),
  );
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        prompt: "PC05E-ORIGINAL-EXECUTE — WAVE 1 PACKAGING ORIGINAL PARITY APPLY",
        run_id: runId,
