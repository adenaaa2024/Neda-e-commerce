/**
 * PHASE-8C-RLS-VIEWS-SECURITY-INVOKER-STAGING
 *
 *   npx tsx scripts/phase8c-rls-views-security-invoker-staging.ts --run-id=<UTC>
 *   npx tsx scripts/phase8c-rls-views-security-invoker-staging.ts --run-id=<UTC> --apply
 *   npx tsx scripts/phase8c-rls-views-security-invoker-staging.ts --run-id=<UTC> --apply --skip-build
 */
...
const APPROVAL_PATH = ".cursor/operator-approvals/phase8c-rls-views-security-invoker-staging-approval.md";
const OUT_BASE = ".cursor/audit-reports/phase8c-rls-views-security-invoker-staging";

/** Dependency-safe apply order (bases before dependents). */
const TARGET_VIEWS = [
...
function readApproval(): { ok: boolean; detail: string } {
  ...
  const phase = /APPROVED_PHASE8C_VIEWS_SECURITY_INVOKER\s*=\s*true/i.test(text);
  const ref = text.match(/TARGET_SUPABASE_REF\s*=\s*(\S+)/)?.[1] ?? "";
  return { ok: run && phase && ref === STAGING_REF, detail: `run=${run} phase=${phase} ref=${ref || "(unset)"}` };
}
...
async function buildUserClient(url: string, anon: string, serviceKey: string) {
  ...
  const { data: authUser } = await admin.auth.admin.listUsers({ perPage: 5 });
  const userId = authUser?.users.find((u) => u.email === email)?.id;
...
async function runRlsLeakTest(
  ...
  const anonClient = createClient(
    process.env.STAGING_SUPABASE_URL?.trim() || process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.STAGING_ANON_KEY?.trim() || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    { auth: { persistSession: false } },
  );
...
async function main(): Promise<void> {
  ...
  if (apply && !approval.ok) blockers.push(`Approval blocked: ${approval.detail}`);
  ...
  if (refFromSupabaseUrl(stagingUrl) !== STAGING_REF) blockers.push(`Must target staging ${STAGING_REF}`);
  if (getStagingProjectRef({ loadEnv: false }) !== STAGING_REF) blockers.push("Staging project ref mismatch");
  if (stagingDb.includes(ORIGINAL_REF)) blockers.push("Refusing — original ref in DB URL");
  if (apply && !stagingDb) blockers.push("STAGING_DIRECT_POSTGRES_URL required for --apply");

  const client = stagingDb
    ? new pg.Client({ connectionString: stagingDb, ssl: { rejectUnauthorized: false } })
    : null;
  if (!client) blockers.push("No staging DB URL");

  const columnsBefore: Record<string, string[]> = {};
  const viewSqlParts: string[] = [
    "-- PHASE-8C security_invoker view recreate (staging only)",
    "BEGIN;",
  ];
  ...
  fs.writeFileSync(path.join(outDir, "01_security_invoker_views.sql"), applySql + "\n");

  let stagingApplied = false;
  let applyError: string | null = null;
  if (apply && client && blockers.length === 0) {
    if (refFromDbUrl(stagingDb) !== STAGING_REF) blockers.push("DB URL not staging");
    try {
  ...
  const { client: userClient, orgId } = stagingApplied
    ? await buildUserClient(stagingUrl, anon, serviceKey)
  ...
  if (stagingApplied) {
    ...
    const pgHeavy = new pg.Client({ connectionString: stagingDb, ssl: { rejectUnauthorized: false } });
  ...
  const rlsLeak = stagingApplied ? await runRlsLeakTest(admin, userClient, orgId) : { pass: false, detail: {} };

  let buildResult = skipBuild ? "skipped" : "fail";
  if (stagingApplied && !skipBuild) {
    try {
  ...
  const allInvoked =
    stagingApplied &&
    TARGET_VIEWS.every((v) => viewsRecreated[v]?.security_invoker === true);
  ...
  const summary = {
    prompt: "PHASE-8C-RLS-VIEWS-SECURITY-INVOKER-STAGING",
    run_id: runId,
    target_ref: STAGING_REF,
    staging_applied: stagingApplied ? "yes" : apply ? "no" : "dry_run",
    views_recreated: stagingApplied ? Object.keys(viewsRecreated) : [],
    views_security_invoker: viewsRecreated,
    column_parity_ok: columnParityOk ? "yes" : stagingApplied ? "no" : "skipped",
    columns_before: columnsBefore,
    columns_after: columnsAfter,
    scanner_smoke: scannerSmoke ? "pass" : stagingApplied ? "fail" : "skipped",
    scanner_checks: scannerChecks,
    claims_smoke: claimsSmoke ? "pass" : stagingApplied ? "fail" : "skipped",
    claims_checks: claimsChecks,
    rls_leak_test: rlsLeak.pass ? "pass" : stagingApplied ? "fail" : "skipped",
    rls_leak_detail: rlsLeak.detail,
    build_result: buildResult,
    SAFE_TO_APPLY_8C_PRODUCTION: safeProduction ? "yes" : "no",
    blockers,
  ...
  fs.writeFileSync(
    ...
    [
      "# Phase 8C security_invoker views (staging)",
      "",
      `| Field | Value |`,
      `|-------|-------|`,
      `| staging_applied | ${summary.staging_applied} |`,
      ...
      `| build_result | ${buildResult} |`,
      `| SAFE_TO_APPLY_8C_PRODUCTION | ${summary.SAFE_TO_APPLY_8C_PRODUCTION} |`,
    ].join("\n") + "\n",
  );

  console.log(JSON.stringify(summary, null, 2));
  if (apply && !safeProduction) process.exit(1);
}
