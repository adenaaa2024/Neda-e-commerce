/**
 * PHASE-CLAIM-TRID-REFERENCE-GRAPH-REVERIFY-AND-SUBMISSION-UNBLOCK-V1
 *   npx tsx scripts/phase-claim-trid-reference-graph-reverify-and-submission-unblock-v1.ts --run-id=<UTC>
 */
...
const OUT =
  ".cursor/audit-reports/phase-claim-trid-reference-graph-reverify-and-submission-unblock-v1";
...
async function checkClaimSubmissionsSchema(
  client: ReturnType<typeof createClient>,
): Promise<{ claim_case_id_exists: boolean; migration_file: string; probe_error: string | null }> {
  const { error } = await client.from("claim_submissions").select("claim_case_id").limit(1);
  if (!error) {
    return { claim_case_id_exists: true, migration_file: SUBMISSION_MIGRATION, probe_error: null };
  }
  const msg = error.message;
  if (msg.includes("claim_case_id")) {
    return { claim_case_id_exists: false, migration_file: SUBMISSION_MIGRATION, probe_error: msg };
  }
  throw new Error(`claim_submissions schema probe: ${msg}`);
}

async function checkCandidateIdOnEdges(
  client: ReturnType<typeof createClient>,
): Promise<{ candidate_id_column_exists: boolean; probe_error: string | null }> {
  const { error } = await client.from("claim_reference_edges").select("candidate_id").limit(1);
  if (!error) return { candidate_id_column_exists: true, probe_error: null };
  const msg = error.message;
  if (msg.includes("candidate_id")) {
    return { candidate_id_column_exists: false, probe_error: msg };
  }
  throw new Error(`claim_reference_edges schema probe: ${msg}`);
}
...
async function main(): Promise<void> {
  ...
  const edgesSchema = await checkCandidateIdOnEdges(client);
  ...
  const filingPreviewPass = perCaseReports.every(
    (r) =>
      r.filing_packet_preview_edge_count === r.reference_edge_count_readmodel &&
  ...
  const manualHandoffChecks = previewPayload.previews.map((preview) => {
    ...
    const previewKeys = new Set(
      preview.reference_edges.map(
        (e) => `${str(e.reference_kind) || str(e.edge_type)}\u0000${str(e.reference_value)}`,
      ),
    );
    const handoffKeys = new Set(graph.lines.map((l) => `${l.kind}\u0000${l.value}`));
    const matches =
      preview.reference_edges.length === 0
        ? graph.lines.length === 0
        : graph.lines.length > 0 &&
          [...previewKeys].every((k) => handoffKeys.has(k) || handoffKeys.size >= previewKeys.size);
    return {
  ...
  try {
    execSync(
      "npx tsx scripts/phase-claim-trid-reference-graph-reverify-and-submission-unblock-v1-smoke.ts",
  ...
  const migrationApplied =
    submissionsSchema.claim_case_id_exists && edgesSchema.candidate_id_column_exists;
  ...
  const claimSubmissionsMigrationStatus = {
    ...
    phase7h_migration_file: PHASE7H_MIGRATION,
    phase7h_candidate_id_exists: edgesSchema.candidate_id_column_exists,
    phase7h_migration_applied: edgesSchema.candidate_id_column_exists ? "yes" : "no",
    note: "User checklist item 20260918120000 refers to claim_submissions anchor migration",
  };
  ...
  const shipmentSample = perCaseReportsWithExport.find(
    (r) => r.family_key_v3 === "removal_shipment_missing",
  );
  const orderSample = perCaseReportsWithExport.find(
    (r) => r.family_key_v3 === "removal_order_discrepancy",
  );

  const results = {
    prompt: "PHASE-CLAIM-TRID-REFERENCE-GRAPH-REVERIFY-AND-SUBMISSION-UNBLOCK-V1",
    ...
    prerequisite_status: {
      ...
      phase7h_legacy_found: phase7hLegacy.found,
      phase7h_legacy_path: phase7hLegacy.path,
    ...
    NEXT_PROMPT:
      safeExecuteSubmission === "yes"
        ...
        : safePlanSubmission === "yes" && !migrationApplied
          ? "Apply migrations 20260917130000 + 20260918120000_phase_claim_submission_record_pilot_v1_anchor on original; then re-run this phase"
          : safePlanSubmission === "yes" && (!submissionApprovalYes || !schemaMigrationApprovalYes)
            ? "Maysam approval: set APPROVED_CLAIM_SUBMISSION_RECORD_PILOT_V1=yes and APPROVED_CLAIM_SUBMISSIONS_SCHEMA_MIGRATION_V1=yes in claim-submission-record-pilot-v1-approval.md"
            : !prereq7hPass || !materializedPass
              ? "PHASE-7H-CLAIM-REFERENCE-EDGE-MATERIALIZATION-PILOT-ORIGINAL-EXECUTE — approval + --execute first"
              : "PHASE-CLAIM-TRID-REFERENCE-GRAPH-REVERIFY-AND-SUBMISSION-UNBLOCK-V1 — remediate graph/export checks",
  };
  ...
  fs.writeFileSync(
    ...
    `# TRID reference graph reverify + submission unblock V1
...
| Submission approval | ${submissionRecordApprovalStatus.approved} |
| Schema migration approval | ${schemaMigrationApprovalStatus.approved} |
| SAFE_TRID_REFERENCE_GRAPH_VERIFIED | ${safeGraph} |
| SAFE_TO_EXECUTE_CLAIM_SUBMISSION_RECORD_PILOT | ${safeExecuteSubmission} |
`,
  );
