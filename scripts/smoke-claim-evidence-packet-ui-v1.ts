...
function main(): void {
  ...
  const verifyOk = (() => {
    ...
    return (
      j.SAFE_EVIDENCE_PACKET_PREVIEW_VERIFIED === "yes" &&
  ...
  const checks = {
    ...
    view_passes_fetch_json: viewSrc.includes("fetchJson={fetchJson}") && viewSrc.includes("organizationId={organizationId}"),
    preview_button: sectionSrc.includes("EVIDENCE_PACKET_PREVIEW_BUTTON_LABEL"),
    html_preview_pane:
      sectionSrc.includes("composeClaimEvidencePacketAction") &&
      sectionSrc.includes("iframe") &&
      sectionSrc.includes("EVIDENCE_PACKET_HTML_PREVIEW_BUTTON_LABEL"),
    evidence_packet_api:
    ...
    drawer_fields_contract: uiContractSrc.includes("intake_run_id"),
    intake_run_id_display: sectionSrc.includes("Intake run ID"),
