/**
 * PC05F — Wave 2 activated rows original parity plan (read-only).
 *
 *   npx tsx scripts/pc05f-wave2-original-parity-plan.ts
 */
...
const EXPECTED_ACTIVATED = 200;
const BATCH_TAG = `PC05D_WAVE2_${WAVE2_EXECUTE_RUN}`;
const APPROVAL_REL = ".cursor/operator-approvals/product-packaging-wave2-original-parity-pc05f-approval.md";
const OUT_BASE = ".cursor/audit-reports/pc05f-wave2-original-parity-plan";
...
function approvalFileContent(): string {
  return `# Product packaging Wave 2 — original parity (PC05F)
...
| Staging source | Wave 2 activated cohort only (\`${BATCH_TAG}\`) |
| Scope | 200 activated PIM case-pack rows — excludes 191 pilot + 50 Wave 1 |

\`\`\`text
APPROVED_TO_RUN_ORIGINAL=false
APPROVED_PRODUCT_PACKAGING_WAVE2_ORIGINAL_PARITY=false
\`\`\`
...
async function main(): Promise<void> {
    ...
    (() => {
      try {
        ...
        return (
          ...
          (m.dimensions_current_after ?? 0) === 441
        );
      } catch {
        return false;
      }
    })();
  if (!activateOk) blockers.push(`Wave2 activate proof required: ${WAVE2_ACTIVATE_RUN}`);
  ...
  if (blockers.length === 0) {
    ...
    if (stagingWave2.size !== EXPECTED_ACTIVATED) {
      blockers.push(`Expected ${EXPECTED_ACTIVATED} activated Wave2 rows on staging, got ${stagingWave2.size}`);
    }
    ...
    const productIds = [...new Set([...stagingWave2.values()].map((r) => r.product_id))];
  ...
  const counts = {
    activated_staging_rows: stagingWave2.size,
  ...
  const insertPlan = diffRows
    ...
    .map((d) => {
      const s = stagingWave2.get(d.candidate_id)!;
      return {
        ...
        evidence_summary: {
          ...s.evidence_summary,
          pc05f_wave2_original_plan_run_id: runId,
        ...
        display_label: `PC05F_WAVE2_ORIGINAL_<run_id>`,
      };
    });
  ...
  fs.writeFileSync(
    path.join(outDir, "wave2-original-parity-plan.md"),
    [
      "# Wave 2 original parity plan — PC05F (plan only)",
      "",
      `**Run id:** \`${runId}\``,
      `**Branch:** \`${branch}\``,
      `**Staging:** \`${STAGING_REF}\` — activated Wave 2 only`,
      ...
      `- Wave2 execute: \`${WAVE2_EXECUTE_RUN}\` | activate: \`${WAVE2_ACTIVATE_RUN}\``,
      "- Excludes 191 pilot (PC05C) and 50 Wave 1 (PC05E)",
      "",
      ...
      "",
      `Expected original current after execute: **441** (191 + 50 + 200)`,
      "",
      "## Classification (200 activated Wave 2 rows)",
      "",
      "| Class | Count |",
      "|-------|------:|",
      ...
      "## Execute note",
      "",
      "Mirror staging **active** state with `effective_from=now()`; tag `PC05F_WAVE2_ORIGINAL_<execute_run_id>`.",
    ].join("\n"),
  );
  ...
  fs.writeFileSync(
    ...
    [
      "# Conflict report — Wave 2 original parity",
      "",
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      "```sql",
      `-- DELETE FROM public.product_packaging_profiles WHERE display_label = 'PC05F_WAVE2_ORIGINAL_<execute_run_id>';`,
      "```",
    ].join("\n"),
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        prompt: "PC05F — WAVE 2 PACKAGING ORIGINAL DATA PARITY PLAN",
        run_id: runId,
        branch,
        wave2_execute_run: WAVE2_EXECUTE_RUN,
        wave2_activate_run: WAVE2_ACTIVATE_RUN,
