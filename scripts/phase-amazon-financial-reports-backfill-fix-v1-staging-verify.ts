/**
 * PHASE-AMAZON-FINANCIAL-REPORTS-BACKFILL-FIX-V1 — staging verify
 *   npx tsx scripts/phase-amazon-financial-reports-backfill-fix-v1-staging-verify.ts
 *   npx tsx scripts/phase-amazon-financial-reports-backfill-fix-v1-staging-verify.ts --execute
 */
...
const OUT_BASE = ".cursor/audit-reports/phase-amazon-financial-reports-backfill-fix-v1";
...
async function main(): Promise<void> {
  ...
  process.env.DIRECT_POSTGRES_URL = process.env.STAGING_DIRECT_POSTGRES_URL?.trim() ?? "";
  ...
  const reimbBefore = await census(client, "amazon_reimbursements", "approval_date");
  const settBefore = await census(client, "amazon_settlements", "posted_date");
  ...
  if (execute) {
    ...
    reimbursementRuns.push(
      await runWorkerUntilDone("reimbursements_30d", runReimbursementsReportsWorker, w30.start, w30.end),
    );

    for (const [i, chunk] of chunkDateWindow(w30.start, w30.end, 30).entries()) {
      settlementRuns.push(
        await runWorkerUntilDone(`settlements_30d_chunk_${i + 1}`, runSettlementReportsWorker, chunk.start, chunk.end),
      );
    }
  }

  const reimbAfter = await census(client, "amazon_reimbursements", "approval_date");
  const settAfter = await census(client, "amazon_settlements", "posted_date");
  const ccAfter = await claimCount(client);
  const dup = await duplicateCheck(client);
  const uploads = await uploadStates(client);
  ...
  let feeAdjusted = execute ? await feeAdjustedSmoke() : { skipped: true };
  ...
  if (execute) {
    ...
    if (smokeResult === "pending") {
      ...
      try {
        ...
        smokeResult = smokeResult === "pending" ? "pass" : smokeResult;
      } catch (e) {
  ...
  const eventFresh =
    execute &&
    (reimbAfter.max_event_date ?? "") >= (reimbBefore.max_event_date ?? "") &&
    (settAfter.max_event_date ?? "") >= (settBefore.max_event_date ?? "");
  ...
  const report = {
    ...
    root_cause: {
      settlement_list_reports_http_400:
        "getReports createdSince/createdUntil exceeded Amazon 90-day retention when backfill used single 7-month window",
      reimbursement_report_fatal_duplicate_windows:
        "Second backfill pass used shifted 30d chunk boundaries → new idempotency keys → duplicate createReport; some returned FATAL",
      event_date_stale_appearance:
        "SP-API mapper stored approval_date/posted_date/amount_total in raw_data only — physical columns null so freshness queries looked stale despite fresh raw_data",
    },
    files_changed: [
      ...
      "scripts/phase-amazon-financial-reports-backfill-fix-v1-staging-verify.ts",
      "scripts/test-reports-api-settlement-list-window.ts",
    ],
    env_flags_after_execute: execute
      ? {
      ...
      : null,
    reimbursement_backfill_result: reimbursementRuns,
    settlement_backfill_result: settlementRuns,
    row_counts_before_after: {
      amazon_reimbursements: { before: reimbBefore.row_count, after: reimbAfter.row_count },
      amazon_settlements: { before: settBefore.row_count, after: settAfter.row_count },
    },
    max_event_dates_before_after: {
      amazon_reimbursements: { before: reimbBefore.max_event_date, after: reimbAfter.max_event_date },
      amazon_settlements: { before: settBefore.max_event_date, after: settAfter.max_event_date },
    },
    raw_upload_state_result: uploads,
    duplicate_check: dup,
    observed_reimbursement_lane_result: observed,
    fee_adjusted_estimate_impact: feeAdjusted,
    no_claim_candidate_mutation_verification: { delta: ccAfter - ccBefore },
    no_scanner_change_verification: { operator_mobile_touched: operatorMobileTouched },
    build_result: buildResult,
    smoke_result: smokeResult,
    event_date_freshness_improved: eventFresh,
    ...
    original_apply_steps_if_approved: [
      ...
      "Deploy mapper + settlement window clamp + failed-upload retry",
      "Run 7d then 30d chunked settlement backfill on original with run_pipeline:true",
      "Verify max(approval_date) and max(posted_date) advance; claim_candidates delta 0",
    ],
    NEXT_PROMPT:
      "PHASE-AMAZON-FINANCIAL-REPORTS-BACKFILL-FIX-V1-ORIGINAL-APPLY — after Maysam approval",
  };

  fs.writeFileSync(path.join(outDir, "manifest.json"), JSON.stringify(report, null, 2));
  fs.writeFileSync(
    path.join(outDir, "summary.md"),
    `# PHASE-AMAZON-FINANCIAL-REPORTS-BACKFILL-FIX-V1\n\n**Run:** ${runId}\n**SAFE:** ${report.SAFE_TO_APPLY_ORIGINAL}\n`,
  );
  console.log(JSON.stringify({ ok: pass, outDir, SAFE: report.SAFE_TO_APPLY_ORIGINAL }));
