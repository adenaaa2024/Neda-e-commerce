/**
 * PC05D-WAVE2-ACTIVATE — Activate eligible Wave 2 versions on staging (approval-gated).
 *
 *   npx tsx scripts/pc05d-wave2-activate-staging.ts --apply
 */
...
const EXECUTE_RUN = "20260526T202000Z";
const REVIEW_RUN = "20260526T203000Z";
const EXPECTED_ELIGIBLE = 200;
const EXPECTED_CURRENT_BEFORE = 241;
const EXPECTED_CURRENT_AFTER = 441;
const BATCH_TAG = `PC05D_WAVE2_${EXECUTE_RUN}`;
const APPROVAL_PATH = ".cursor/operator-approvals/product-packaging-backfill-pc05d-scale-activate-approval.md";
const EXECUTE_DIR = `.cursor/audit-reports/pc05d-wave2-packaging-scale-staging-execute/${EXECUTE_RUN}`;
const REVIEW_DIR = `.cursor/audit-reports/pc05d-wave2-review-census-staging/${REVIEW_RUN}`;
const OUT_BASE = ".cursor/audit-reports/pc05d-wave2-activate-staging";
...
async function main(): Promise<void> {
  ...
  if (fs.existsSync(executeManifestPath)) {
    try {
      ...
      if (!em.ok || (em.inserted_versions ?? 0) !== EXPECTED_ELIGIBLE) {
        blockers.push("Wave2 execute manifest not PASS 200");
      }
    } catch {
  ...
  if (fs.existsSync(reviewManifestPath)) {
    try {
      ...
      if (!rm.ok || (rm.activate_eligible_count ?? 0) !== EXPECTED_ELIGIBLE) {
        blockers.push("Wave2 review manifest not PASS 200 eligible");
      }
  ...
  for (const vid of eligibleVersionIds) {
    if (!allWaveVersionIds.has(vid)) blockers.push(`Eligible version not in wave2 execute: ${vid}`);
  }
  ...
  fs.writeFileSync(
    ...
    [
      "# Approval proof — PC05D Wave 2 activate",
      "",
  ...
  fs.writeFileSync(
    ...
    [
      "# Activation result — PC05D Wave 2",
      "",
      `Run: \`${runId}\``,
      `Apply: **${apply ? "YES" : "NO"}**`,
      "",
  ...
  fs.writeFileSync(
    ...
    [
      "-- PC05D Wave 2 activate rollback — revert to needs_review",
      `-- Activate run: ${runId}`,
      `-- Batch: ${BATCH_TAG}`,
      "",
      "BEGIN;",
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        prompt: "PC05D-WAVE2-ACTIVATE — PACKAGING SCALE WAVE 2 REVIEW ACTIVATE STAGING",
        run_id: runId,
