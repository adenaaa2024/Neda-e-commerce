/**
 * PC05F-ORIGINAL-EXECUTE — Apply Wave 2 packaging parity to original/current (approval-gated).
 *
 *   npx tsx scripts/pc05f-wave2-original-parity-execute.ts --apply
 *   npx tsx scripts/pc05f-wave2-original-parity-execute.ts --apply --plan-run-id=20260526T205000Z
 */
...
const PLAN_DEFAULT = "20260526T205000Z";
const APPROVAL_PATH = ".cursor/operator-approvals/product-packaging-wave2-original-parity-pc05f-approval.md";
const PLAN_BASE = ".cursor/audit-reports/pc05f-wave2-original-parity-plan";
const OUT_BASE = ".cursor/audit-reports/pc05f-wave2-original-parity-execute";
const EXPECTED_ROWS = 200;
const EXPECTED_CURRENT_AFTER = 441;
...
function readApprovalFlags(): { run: boolean; parity: boolean; raw: Record<string, string> } {
  ...
  const parM = text.match(/APPROVED_PRODUCT_PACKAGING_WAVE2_ORIGINAL_PARITY\s*=\s*(\S+)/);
  const runVal = runM?.[1] ?? "";
  const parVal = parM?.[1] ?? "";
  return {
    run: runVal === "true",
    ...
    raw: {
      ...
      APPROVED_PRODUCT_PACKAGING_WAVE2_ORIGINAL_PARITY: parVal,
    },
  };
}
...
async function runPostInsertSmoke(
  ...
  for (const row of sample.length ? sample : insertedIds.slice(0, 1)) {
    ...
    const ver = await client.query(
      `SELECT profile_status, evidence_summary->>'pc05_candidate_id' AS cid,
              evidence_summary->>'pc05f_original_execute_run_id' AS exec_run
    ...
    if ((ver.rowCount ?? 0) !== 1) failures.push(`${row.candidate_id}: version missing`);
      ...
      if (ver.rows[0].exec_run !== executeRunId) failures.push(`${row.candidate_id}: missing pc05f execute run id`);
    }
  ...
  if ((tagged.rows[0]?.c ?? 0) < insertedIds.length) {
...
async function main(): Promise<void> {
  ...
  const batchTag = `PC05F_WAVE2_ORIGINAL_${runId}`;
  ...
  fs.writeFileSync(
    ...
    [
      "# Approval proof — PC05E Wave 1 original execute",
      "",
      `File: \`${APPROVAL_PATH}\``,
      "```text",
      `APPROVED_TO_RUN_ORIGINAL=${approval.raw.APPROVED_TO_RUN_ORIGINAL}`,
      `APPROVED_PRODUCT_PACKAGING_WAVE2_ORIGINAL_PARITY=${approval.raw.APPROVED_PRODUCT_PACKAGING_WAVE2_ORIGINAL_PARITY}`,
      "```",
  ...
  fs.writeFileSync(
    ...
    [
      "-- PC05F Wave 2 original parity rollback",
    ...
    if (hardBlockers.length === 0) {
      ...
      for (const row of insertPlan) {
        ...
        const evidence = {
          ...
          pc05f_original_execute_run_id: runId,
          pc05f_wave2_original_batch_tag: batchTag,
  ...
  fs.writeFileSync(
    ...
    [
      "# Execute result — PC05F Wave 2 original",
      "",
      `Apply: **${apply ? "YES" : "NO"}**`,
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      `Expected current after: **${EXPECTED_CURRENT_AFTER}** (191 pilot + 50 wave1 + 200 wave2)`,
    ].join("\n"),
  );
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        prompt: "PC05F-ORIGINAL-EXECUTE — WAVE 2 PACKAGING ORIGINAL PARITY APPLY",
        run_id: runId,
