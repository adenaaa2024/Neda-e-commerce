 ...
 *
 * Governed Amazon SP-API catalog lookup + product/map promotion for V202 API cohort.
 * Does not update expected_packages. Staging only.
 *
 *   npx tsx scripts/expected-packages-amazon-api-evidence-execute-v202.ts --run-id=<id>
 *   npx tsx scripts/expected-packages-amazon-api-evidence-execute-v202.ts --dry-run-id=20260522T200000Z
 */
...
const DRY_RUN_BASE = ".cursor/audit-reports/expected-packages-amazon-api-evidence-dry-run-v202";
const OUT_BASE = ".cursor/audit-reports/expected-packages-amazon-api-evidence-execute-v202";
const MATCH_SOURCE = "expected_packages_amazon_api_evidence_v202";
...
function dryRunIdArg(): string {
  ...
  return a ? a.split("=")[1]!.trim() : "20260522T200000Z";
}
...
function loadExecuteCohort(dryRunId: string): { cohort: CohortRow[]; bundles: AsinBundle[] } {
  const dryDir = path.join(process.cwd(), DRY_RUN_BASE, dryRunId);
  const cohortPath = path.join(dryDir, "api-evidence-cohort.json");
  const linesPath = path.join(dryDir, "dry-run-lines.json");
  if (!fs.existsSync(cohortPath) || !fs.existsSync(linesPath)) {
    throw new Error(`Missing dry-run artifacts under ${dryDir}`);
  }
  const cohortFile = JSON.parse(fs.readFileSync(cohortPath, "utf8")) as { rows: CohortRow[] };
  ...
  const executeRows = cohort.filter((r) => callAsins.has(r.asin.trim().toUpperCase()));
  return { cohort, bundles: groupByAsin(executeRows) };
}
...
async function main(): Promise<void> {
  ...
  const { cohort, bundles } = loadExecuteCohort(dryRunId);
  if (bundles.length === 0) {
    const manifest = {
      prompt: "EXPECTED-PACKAGES-AMAZON-API-EVIDENCE-EXECUTE-V202",
      run_id: runId,
      dry_run_id: dryRunId,
      status: "PASS_NO_API_NEEDED",
      cohort_rows: cohort.length,
      distinct_asins_to_call: 0,
    };
      ...
      const mapExists = await pgClient.query(
        `SELECT 1 FROM public.product_identifier_map
         WHERE organization_id = $1::uuid AND store_id = $2::uuid AND product_id = $3::uuid
           AND deleted_at IS NULL AND UPPER(TRIM(asin)) = $4 LIMIT 1`,
  ...
  const cohortEpIds = new Set(cohort.map((r) => r.expected_package_id));
  const resolvedAfter = await pgClient.query(
    `
    WITH ep AS (
  ...
  const resolvedCount = Number(resolvedAfter.rows[0]?.resolved ?? 0);
  ...
  const manifest = {
    ...
    distinct_asins_executed: bundles.length,
    cohort_rows_total: cohort.length,
    outcomes: { inserted, skipped, failed },
    cohort_ep_read_layer_resolved_after: resolvedCount,
  ...
  fs.writeFileSync(
    ...
    [
      "# Amazon API evidence execute V202",
      "",
      `- **Dry-run:** \`${dryRunId}\``,
      `- **Credential preflight:** PASS`,
      `- **Distinct ASINs executed:** ${bundles.length}`,
      `- **Amazon catalog API calls:** ${apiCallCount}`,
      `- **Products + maps inserted:** ${inserted}`,
      `- **Skipped:** ${skipped}`,
      `- **Failed:** ${failed}`,
      `- **Cohort EP read-resolved after:** ${resolvedCount} / ${cohort.length}`,
      `- **Read-layer (all EP):** ${beforeCoverage.read_layer_resolved} → **${afterCoverage.read_layer_resolved}**`,
      "",
      "Does not update `expected_packages` rows.",
    ].join("\n") + "\n",
  );
  ...
  fs.writeFileSync(
    ...
    [
      "-- Rollback V202 API evidence execute batch only.",
