/**
 * INVENTORY-VIEWS-RETURN-ITEMS-DELETED-AT-FILTER-V189 — Original / Vercel Production DB only.
 *
 *   npx tsx scripts/inventory-views-return-items-deleted-at-filter-v189-original.ts --run-id=<id>
 *   npx tsx scripts/inventory-views-return-items-deleted-at-filter-v189-original.ts --run-id=<id> --apply
 */
...
const APPROVAL_PATH =
  ".cursor/operator-approvals/inventory-views-return-items-deleted-at-filter-v189-original-approval.md";
const OUT_BASE = ".cursor/audit-reports/inventory-views-return-items-deleted-at-filter-v189-original";
const FORBIDDEN = [
...
function readApproval(): boolean {
  return /APPROVED_TO_RUN_ORIGINAL\s*=\s*true/i.test(
...
async function verifyCounts(client: pg.Client): Promise<Record<string, unknown>> {
  ...
  const hasFilter = await client.query(
  ...
  return {
    ...
    view_def_has_deleted_at_filter: /\bdeleted_at\s+IS\s+NULL\b/i.test(
      String(hasFilter.rows[0]?.def ?? ""),
    ),
  };
}
...
async function main(): Promise<void> {
  ...
  const approvalRef = (() => {
    try {
      const m = fs.readFileSync(path.join(process.cwd(), APPROVAL_PATH), "utf8").match(
        /ORIGINAL_PROJECT_REF\s*=\s*([a-z]{20})/i,
      );
      return m?.[1]?.toLowerCase() ?? null;
    } catch {
      return null;
  ...
  const targetOk =
    dbUrl.length > 0 &&
    connRef === ORIGINAL_REF &&
    (approvalRef === null || approvalRef === ORIGINAL_REF);
  ...
  fs.writeFileSync(path.join(outDir, "preflight.json"), JSON.stringify(guardDoc, null, 2));

  if (!targetOk || !preflight.pass || !notStaging || !notMislabeledProd) {
    console.error(JSON.stringify({ ok: false, phase: "preflight", guardDoc }, null, 2));
    process.exit(2);
  }
  ...
  if (!apply) {
    fs.writeFileSync(
      ...
      [
        "# V189 original preflight",
        "",
        `- original ref: \`${ORIGINAL_REF}\` — ${targetOk ? "PASS" : "FAIL"}`,
        `- staging not targeted: ${notStaging ? "PASS" : "FAIL"}`,
        `- migration preflight: ${preflight.pass ? "PASS" : "FAIL"}`,
        "",
        "Run with `--apply` when `APPROVED_TO_RUN_ORIGINAL=true`.",
      ].join("\n"),
    );
  ...
  if (!readApproval()) {
    throw new Error(`Set APPROVED_TO_RUN_ORIGINAL=true in ${APPROVAL_PATH}`);
  }
  ...
  try {
    ...
    const alreadyApplied = Boolean(before.view_def_has_deleted_at_filter);
    ...
    const manifest = {
      prompt: "INVENTORY-VIEWS-V189-ORIGINAL-PREFLIGHT-AND-APPLY",
      run_id: runId,
      original_ref: ORIGINAL_REF,
      migration: MIGRATION,
      mode: alreadyApplied ? "apply_idempotent_skip" : "apply",
    ...
    fs.writeFileSync(
      ...
      [
        "# V189 original apply result",
        "",
        `- Mode: **${manifest.mode}**`,
        `- deleted_at filter in viewdef: **${hasFilter}**`,
        `- active return_items: ${before.active_return_items} → ${after.active_return_items}`,
        `- soft-deleted return_items: ${before.soft_deleted_return_items} → ${after.soft_deleted_return_items}`,
