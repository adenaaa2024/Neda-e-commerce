/**
 * PC05D-WAVE2-REVIEW — Wave 2 packaging review census (read-only, staging).
 *
 *   npx tsx scripts/pc05d-wave2-review-census-staging.ts
 *   npx tsx scripts/pc05d-wave2-review-census-staging.ts --execute-run-id=20260526T202000Z
 */
...
const EXECUTE_DEFAULT = "20260526T202000Z";
...
const OUT_BASE = ".cursor/audit-reports/pc05d-wave2-review-census-staging";
...
function reviewRow(
  ...
  if (!preActivateSnapshot && status !== "needs_review") holdout.push(`status_${status}`);
  if (preActivateSnapshot && status !== "needs_review" && status !== "active") {
    holdout.push(`status_${status}`);
  }
  ...
  if (!preActivateSnapshot && db.has_current_snapshot) holdout.push("already_has_dimensions_current");
  ...
  return {
    ...
    profile_status: preActivateSnapshot ? "needs_review" : status,
...
async function main(): Promise<void> {
  ...
  const preActivateSnapshot =
    process.argv.includes("--pre-activate-snapshot") ||
    process.env.PC05D_WAVE2_REVIEW_PRE_ACTIVATE === "1";
  const batchTag = `PC05D_WAVE2_${executeRunId}`;
  const executeDir = path.join(
    process.cwd(),
    ".cursor/audit-reports/pc05d-wave2-packaging-scale-staging-execute",
    executeRunId,
  );
  ...
  if (blockers.length === 0 && dbUrl) {
    ...
    for (const row of r.rows as Record<string, unknown>[]) {
      ...
      reviews.push(reviewRow(row, dryMap.get(cid), batchTag, preActivateSnapshot));
    }
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          batch_tag: batchTag,
          dimensions_current_total: curCount.rows[0]?.c,
          expected_dimensions_current_unchanged: 241,
        },
        null,
        2,
      ),
  ...
  fs.writeFileSync(path.join(outDir, "wave2-row-review.json"), JSON.stringify(reviews, null, 2));
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(outDir, "wave2-review-census.md"),
    [
      "# Wave 2 review census — PC05D (read-only)",
      "",
      `**Run id:** \`${runId}\``,
      ...
      preActivateSnapshot ? "| Snapshot | **pre-activate** (structure rules only; status/current ignored) |" : "",
      "",
      ...
      "- `dimensions_current` total unchanged at 241 (Wave 2 not yet activated)",
      "",
      ...
      holdout.length
        ? Object.entries(holdoutByReason)
            ...
            .map(([k, v]) => `- \`${k}\`: ${v}`)
            .join("\n")
        : "None — all 200 eligible for governed activate.",
      "",
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `- Script: scripts/pc05d-wave2-review-census-staging.ts`,
    ].join("\n"),
  );
  ...
  const ok =
    ...
    (preActivateSnapshot ? eligible.length === EXPECTED_ROWS : true);
  fs.writeFileSync(
    path.join(outDir, "manifest.json"),
    JSON.stringify(
      {
        prompt: "PC05D-WAVE2-REVIEW — PACKAGING SCALE WAVE 2 REVIEW CENSUS",
        run_id: runId,
        ...
        read_only: true,
        pre_activate_snapshot: preActivateSnapshot,
