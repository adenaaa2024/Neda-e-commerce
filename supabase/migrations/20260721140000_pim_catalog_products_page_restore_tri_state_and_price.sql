...
-- Restore pim_catalog_products_page tri-state filters + soft-delete (from 20260505140000)
-- and tie-break latest price by created_at. Fixes regressions from boolean-only replacements.
...
COMMENT ON FUNCTION public.pim_catalog_products_page(
  ...
  'PIM catalog grid: org+store scoped active products; tri-state filters; latest product_prices (observed_at then created_at).';
