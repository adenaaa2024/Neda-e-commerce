-- Fix: "relation ranked does not exist" — WITH CTEs only apply to the single SQL statement that follows.
-- The function had two separate SELECTs; the second could not see `ranked`. Combine into one SELECT ... INTO.
