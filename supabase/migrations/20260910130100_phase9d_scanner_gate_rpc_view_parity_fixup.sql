-- Phase 9D view parity fix — operational tracking key + zero-EP row filter
BEGIN;
...
BEGIN
      ...
      FROM (
        SELECT public._scanner_inventory_row_json(
         ...
         WHERE ep.organization_id = p_organization_id
           ...
           AND ep.fnsku = v_code
         GROUP BY
           public._scanner_ep_tracking_key(ep.tracking_number),
    ...
    SELECT 1
     ...
     WHERE ep.organization_id = p_organization_id
       ...
       AND (
         ...
         OR public._scanner_ep_tracking_key(ep.tracking_number) = v_norm
    ...
    SELECT coalesce(jsonb_agg(DISTINCT to_jsonb(p.id::text)), '[]'::jsonb)
     ...
     WHERE p.organization_id = p_organization_id
       ...
       AND (
         ...
         OR public._scanner_ep_tracking_key(p.tracking_number) = v_norm
  ...
  IF EXISTS (
    ...
    WITH ep_grouped AS (
      SELECT
        public._scanner_ep_tracking_key(ep.tracking_number) AS tracking_norm,
    ...
    scanned_grouped AS (
      SELECT
        public._scanner_ep_tracking_key(p.tracking_number) AS tracking_norm,
