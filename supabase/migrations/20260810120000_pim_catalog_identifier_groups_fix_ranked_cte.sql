-- Fix: "relation ranked does not exist" reintroduced in 20260717120000 / 20260719140000.
-- Those versions used two separate SELECT statements after one WITH clause; only the first
-- SELECT sees the CTEs. Merge aggregate + page rows into a single SELECT ... INTO (same fix as 20260516120000).
-- Preserves: join to active products (deleted_at IS NULL), page sizes 25/50/100, optional 10 for hub.
...
COMMENT ON FUNCTION public.pim_catalog_identifier_groups(uuid, uuid, text, integer, integer) IS
  'PIM Hub: paginated identifier groups from product_identifier_map; DISTINCT product_id; excludes soft-deleted products; CTE scope safe.';
