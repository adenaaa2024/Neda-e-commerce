...
-- Tie-break latest price by created_at when observed_at matches.
...
COMMENT ON FUNCTION public.pim_catalog_products_page(
  ...
  'PIM catalog grid: org+store scoped active (non-soft-deleted) products with primary map row, latest price (observed_at then created_at), vendor/category names.';
