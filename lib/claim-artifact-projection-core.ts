/**
 * NEXT-CLAIM-CANONICAL-03 — Artifact-agnostic claim projection core (read-only).
 *
 * Shared by `claim_candidates` and `claim_candidate_drafts` via context adapter.
 * No DB writes beyond SELECT used by callers.
 */
...
export const SOURCE_FETCH_CHUNK = 120;
export const PAGE = 400;
export const PIM_OPEN_PAGE_MAX = 12_000;
export const MAP_PREFETCH_CONCURRENCY = 12;
...
export function claimArtifactStr(v: unknown): string | null {
  return n(v);
}
...
export function extractIdentifierHints(source: Record<string, unknown> | null): IdentifierLookupHints & {
...
export async function fetchSourceRowsByIds(
  ...
  for (let i = 0; i < ids.length; i += SOURCE_FETCH_CHUNK) {
    ...
    const physicalTable = table === "returns" ? "return_items" : table;
    let q = client.from(physicalTable).select("*").in("id", slice);
...
/** Legacy inbox: context view keyed by `claim_candidate_id` (= candidate row id). */
...
export type ClaimArtifactCoreProjection = InboxProjectionRow & {
  source_found: boolean;
  proposed_resolved_product_id: string | null;
};

export type ClaimArtifactContextFetcher = (
  client: SupabaseClient,
  artifactIds: string[],
) => Promise<Map<string, Record<string, unknown>>>;

/**
 * Shared projection for any artifact row shaped like claim_candidates / claim_candidate_drafts
 * (organization_id, store_id, source_table, source_row_id, resolved_product_id, evidence_status, identifiers).
 *
 * @param fetchContext — When null, operational merge uses only the artifact row + source row (draft-safe).
 */
export async function projectClaimArtifactsBatchCore(
  client: SupabaseClient,
  batch: Record<string, unknown>[],
  organizationId: string,
  fetchContext: ClaimArtifactContextFetcher | null,
): Promise<Map<string, ClaimArtifactCoreProjection>> {
  const out = new Map<string, ClaimArtifactCoreProjection>();
  ...
  const artifactIds = batch.map((c) => n(c.id)).filter(Boolean) as string[];
  const contextByArtifactId = fetchContext ? await fetchContext(client, artifactIds) : new Map<string, Record<string, unknown>>();

  const sourcePackByArtifactId = new Map<string, Awaited<ReturnType<typeof resolveClaimCandidateSourcePack>>>();
  for (const c of batch) {
    const id = n(c.id) ?? "";
    const ctx = contextByArtifactId.get(id) ?? null;
    const pack = await resolveClaimCandidateSourcePack(client, c, sourceMaps, ctx);
    sourcePackByArtifactId.set(id, pack);
  }
  ...
  for (const c of batch) {
    ...
    const pack = sourcePackByArtifactId.get(n(c.id) ?? "") ?? {
      row: null,
      alternateTier: null,
      opReasonCodes: [],
  ...
  for (const c of batch) {
    ...
    if (existingResolved) {
      ...
      const pack = sourcePackByArtifactId.get(artifactId) ?? {
        row: null,
        alternateTier: null,
        opReasonCodes: [],
        ambiguousOperational: false,
