...
import { resolveProductForScannerItem } from "./scanner/resolve-product-for-scanner-item";
...
export async function resolveScannerProductIdentifiers(
  ...
  if (legacy && res.resolved_product_id && legacy !== res.resolved_product_id) {
    return {
      resolved_product_id: null,
      resolved_catalog_product_id: null,
  ...
  if (res.status === "resolved" && res.resolved_product_id) {
    return {
  ...
  return {
    ...
    identifier_resolution_confidence: res.confidence > 0 ? res.confidence : null,
  };
}
