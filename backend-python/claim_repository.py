...
repo.get_return_item_by_id(return_item_id, organization_id) -> dict | None
...
from claim_agent_constants import CLAIM_LINES_SELECT, CLAIM_SUBMISSIONS_WITH_RETURN_ITEM
...
class ClaimRepository:
    ...
    def get_submissions(
        ...
        *,
        ...
        -------
        list[dict]
            Raw Supabase row dicts (may include embedded ``return_items``).
        """
    ...
    def get_return_for_submission(
        ...
        """
        Fetch the ``return_items`` row linked to a ``claim_submissions`` row.

        ``claim_submissions.return_id`` → ``return_items.id`` (approved spine).

        Returns
        -------
        dict | None
            The embedded ``return_items`` object with at least ``id``, ``order_id``,
            ``asin``, ``fnsku``, ``sku``, ``lpn`` when present; *None* if the
            submission is missing or has no linked item.
        """
        effective_org = (organization_id or self.org_id or "").strip()
        ...
        try:
            q = (
                self.supabase.table("claim_submissions")
                .select(f"id, return_items(id, order_id, asin, fnsku, sku, lpn)")
                .eq("id", sid)
            )
            if effective_org:
                q = q.eq("organization_id", effective_org)
    ...
    def get_return_item_by_id(
        self,
        return_item_id: str,
        organization_id: str | None = None,
    ) -> dict[str, Any] | None:
        """
        Load a single ``return_items`` row by primary key.

        Used when ``claim_submissions.return_id`` is known but the PostgREST
        embed on the submission row is missing or empty.
        """
        effective_org = (organization_id or self.org_id or "").strip()
        rid = (return_item_id or "").strip()
        if not rid:
            return None
        try:
            q = (
                self.supabase.table("return_items")
                .select("id, order_id, asin, fnsku, sku, lpn, item_name")
                .eq("id", rid)
            )
            if effective_org:
                q = q.eq("organization_id", effective_org)
            ...
            _log(
                f"get_return_item_by_id failed for {rid!r}: {type(exc).__name__}: {exc}"
            )
            raise

    def get_claim_line_for_return_item(
        self,
        return_item_id: str,
        organization_id: str | None = None,
    ) -> dict[str, Any] | None:
        """Optional ``claim_lines`` row for item-grain filing context."""
        effective_org = (organization_id or self.org_id or "").strip()
        rid = (return_item_id or "").strip()
        if not rid:
            return None
        try:
            q = (
                self.supabase.table("claim_lines")
                .select(CLAIM_LINES_SELECT)
            ...
            _log(f"get_claim_line_for_return_item non-fatal: {type(exc).__name__}: {exc}")
            return None
    ...
    def resolve_amazon_order_id_from_row(row: dict[str, Any]) -> str | None:
        ...
        4. ``return_items.order_id`` (embedded or fallback via ``return_id``)
        5. ``claim_lines.order_id`` when a line exists for ``return_id``
        ...
        if isinstance(item, dict):
            ...
            if isinstance(oid, str) and oid.strip():
                s = oid.strip()
                _log(f"Resolved amazon_order_id from return_items.order_id: {s!r}")
                return s

        line = row.get("claim_lines")
        ...
        if isinstance(line, dict):
            ...
            if isinstance(oid, str) and oid.strip():
                s = oid.strip()
                _log(f"Resolved amazon_order_id from claim_lines.order_id: {s!r}")
                return s
