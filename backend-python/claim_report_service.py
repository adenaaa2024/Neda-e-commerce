...
# reportlab ΓÇö PDF generation (pip install reportlab)
...
def _safe_text(val: Any, fallback: str = "N/A", max_len: int = 2000) -> str:
    ...
    * Truncates to *max_len* characters
    ...
    if len(cleaned) > max_len:
        cleaned = cleaned[:max_len] + "ΓÇª [truncated]"
    return cleaned
...
def _extract_bulk_report_fields(claim_data: dict[str, Any]) -> dict[str, Any]:
    ...
    claim_id = _safe_text(
        claim_data.get("id") or claim_data.get("submission_id"), fallback="ΓÇö"
    )

    asin  = _safe_text(claim_data.get("asin")  or payload.get("asin"),  fallback="ΓÇö")
    sku   = _safe_text(claim_data.get("sku")   or payload.get("sku"),   fallback="ΓÇö")
    fnsku = _safe_text(claim_data.get("fnsku") or payload.get("fnsku"), fallback="ΓÇö")
    ...
    carrier = _safe_text(
        payload.get("carrier") or payload.get("carrier_name"), fallback="ΓÇö"
    )

    amount_raw   = _safe_text(
        payload.get("amount") or payload.get("reimbursement_amount")
        or claim_data.get("amount"), fallback="ΓÇö",
    )
    ...
    case_id     = _safe_text(
        claim_data.get("amazon_case_id") or claim_data.get("case_id"), fallback="ΓÇö"
    )
    link_status = _safe_text(payload.get("link_status"), fallback="ΓÇö")
...
# ---------------------------------------------------------------------------
# Supabase logo download (PDF asset only ΓÇö no DB sync)
# ---------------------------------------------------------------------------
...
def _download_logo_from_supabase(
    ...
    if client is None:
        ...
        if not (url and key):
            _log("Logo: missing Supabase credentials ΓÇö skipping logo download.")
            return None
        try:
    ...
    # ΓöÇΓöÇ Download raw bytes ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    try:
    ...
    _log(f"Logo: download succeeded ΓÇö {len(raw)} bytes received.")
    ...
    _log(f"Logo: raw temp file ΓåÆ {raw_tmp!r}  size={raw_size} bytes")

    if raw_size == 0:
        _log("Logo: raw temp file is empty ΓÇö aborting.")
        try:
            os.remove(raw_tmp)
        except OSError:
            pass
    ...
    # ΓöÇΓöÇ Normalise to PNG via Pillow so ReportLab can always render it ΓöÇΓöÇΓöÇΓöÇΓöÇ
    try:
        ...
        with _PILImage.open(raw_tmp) as im:
            _log(f"Logo: PIL opened ΓÇö mode={im.mode} size={im.size}")
            # Convert palette / RGBA ΓåÆ RGB so JPEG-derived images save cleanly
            if im.mode not in ("RGB", "L"):
                im = im.convert("RGB")
            im.save(png_tmp, format="PNG")

        png_size = os.path.getsize(png_tmp)
        _log(f"Logo: normalised PNG ΓåÆ {png_tmp!r}  size={png_size} bytes")
        ...
        if png_size == 0:
            _log("Logo: normalised PNG is empty ΓÇö falling back to raw file.")
            return raw_tmp  # last resort

        return png_tmp

    except ImportError:
        _log("Logo: Pillow not installed ΓÇö using raw JPEG directly.")
        return raw_tmp
    except Exception as exc:
        _log(f"Logo: PIL normalisation failed ({exc}) ΓÇö using raw JPEG as fallback.")
        return raw_tmp
...
def _draw_bulk_report_chrome(
    ...
    # ΓöÇΓöÇ Logo ΓÇö left side ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    logo_w = 2.0 * cm
    ...
    if logo_path and os.path.isfile(logo_path):
        ...
        _log(
            f"Logo draw started ΓÇö path={logo_path!r} "
            f"exists=True size={logo_size} bytes "
            f"pos=({logo_x:.1f},{logo_y:.1f}) dim=({logo_w:.1f}├ù{logo_h:.1f})"
        )
        if logo_size > 0:
            try:
                canvas.drawImage(
            ...
            _log("Logo draw skipped ΓÇö file is empty; using text fallback.")
            _draw_amazon_text(canvas, logo_x, logo_y + logo_h * 0.35)
    else:
        _log(
            f"Logo draw skipped ΓÇö path={logo_path!r} "
            f"exists={os.path.isfile(logo_path) if logo_path else False}; "
            "using text fallback."
        )
        _draw_amazon_text(canvas, logo_x, logo_y + logo_h * 0.35)

    # ΓöÇΓöÇ Company name + subtitle ΓÇö immediately right of logo ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Marketplace badge ΓÇö right side ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    badge_w = 2.4 * cm
    badge_h = 0.38 * cm
...
def generate_claim_report(
    ...
    ----------
    claim_data:
        Any dict that carries claim information ΓÇö typically a
    ...
    """
    _log("Generating claim PDF reportΓÇª")

    out_dir = output_dir or _PDF_OUTPUT_DIR
    try:
        os.makedirs(out_dir, exist_ok=True)
    except OSError as exc:
        _log(f"Cannot create PDF output dir {out_dir!r}: {exc} ΓÇö falling back to tempdir.")
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=2.5 * cm,
        bottomMargin=2.5 * cm,
        title=f"Claim Report ΓÇö {order_id}",
    ...
    story.append(Paragraph("Amazon Seller Central ΓÇö Claim Report", style_title))
    story.append(
        Paragraph(
    ...
    tbl_data = [
        [
            ...
            Paragraph(_safe_text(val) if val != "N/A" else "ΓÇö", style_na if val == "N/A" else style_value),
        ]
    ...
    for key in detail_keys:
        ...
        story.append(Paragraph(escaped_br if val != "N/A" else "ΓÇö", st))
...
def generate_bulk_report_pdf(
    ...
    * **Page 1** ΓÇö Master summary: batch-selection header, intro paragraph,
      and a line-item table (Item Name / Order ID / Defect Type / Status /
      Photo / Detail).
    * **Page 2** ΓÇö Detail view: Identifiers (ASIN, FNSKU, SKU), Logistics
    ...
    """
    _log("Generating bulk-report PDF (claims-bulk-report format, 2 pages)ΓÇª")
    ...
    # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ PAGE 1 ΓòÉΓòÉ
    ...
    # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ PAGE 2 ΓòÉΓòÉ
    story.append(Paragraph("Claim evidence package", sty_pkg_title))
    story.append(
        Paragraph(
...
# ===========================================================================
# ΓöÇΓöÇ REUSABLE SINGLE-ENTRY PDF SERVICE ΓöÇΓöÇ
# ===========================================================================
...
def _pick_item_photos(ret: dict[str, Any]) -> dict[str, str]:
    ...
    Recognised slot names ΓåÆ returned keys:
...
def _pick_package_photos(pkg: dict[str, Any]) -> dict[str, str]:
    ...
    Sources:
    * ``photo_evidence`` (dict) ΓåÆ outer_box_photo_url, box_label_photo_url
    * ``manifest_photo_url``    ΓåÆ packing_slip_photo_url
...
def _load_claim_pdf_context(
    ...
    """
    Load a full ``claim_submissions`` row ΓÇö including linked ``returns``,
    ``packages``, and ``pallets`` rows ΓÇö from Supabase.

    Relationship chain:
        claim_submissions.return_id ΓåÆ returns.id
        returns.package_id          ΓåÆ packages.id
        returns.pallet_id           ΓåÆ pallets.id
    ...
    if isinstance(ret_embed, dict) and ret_embed:
        ...
        _log(
            f"_load_claim_pdf_context: returns embedded ΓÇö "
        ...
        if ret_id:
            _log(f"_load_claim_pdf_context: returns embed empty ΓÇö fetching by return_id={ret_id!r}")
            try:
                rr = (
    ...
    # Load packages row via returns.package_id (NOT submission_id ΓÇö that column doesn't exist)
    ...
    if pkg_id:
        try:
            ...
            if pkgs:
                row["_package"] = pkgs[0]
                _log(
                    f"_load_claim_pdf_context: package loaded ΓÇö "
        ...
        _log("_load_claim_pdf_context: returns.package_id is empty ΓÇö skipping packages lookup")
    ...
    if pallet_id:
        try:
            ...
            if plts:
                row["_pallet"] = plts[0]
                _log(
                    f"_load_claim_pdf_context: pallet loaded ΓÇö "
                    f"number={plts[0].get('pallet_number')!r}"
                )
        except Exception as exc:
            _log(f"_load_claim_pdf_context: pallets query failed (non-fatal): {exc}")
    else:
        _log("_load_claim_pdf_context: returns.pallet_id is empty ΓÇö skipping pallets lookup")
...
def _normalize_claim_pdf_context(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    * ``_full_return``  ΓÇö returns row loaded by ``_load_claim_pdf_context``
    * ``raw``           ΓÇö claim_submissions columns (claim_amount, amazon_case_id, status ΓÇª)
    * ``source_payload``ΓÇö JSONB bag of extra fields
    * ``_package``      ΓÇö packages row (tracking_number, carrier_name, package_number)
    * ``_pallet``       ΓÇö pallets row  (pallet_number)

    All missing fields default to ``"ΓÇö"``.
    """
    ...
    def _str(*values: Any, fallback: str = "ΓÇö") -> str:
    ...
    # ΓöÇΓöÇ Core identifiers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    claim_id = _str(raw.get("id"), raw.get("submission_id"), fallback="ΓÇö")
    ...
    asin  = _str(full_ret.get("asin"),  raw.get("asin"),  payload.get("asin"),  fallback="ΓÇö")
    fnsku = _str(full_ret.get("fnsku"), raw.get("fnsku"), payload.get("fnsku"), fallback="ΓÇö")
    sku   = _str(full_ret.get("sku"),   raw.get("sku"),   payload.get("sku"),   fallback="ΓÇö")

    # ΓöÇΓöÇ Logistics ΓÇö package / pallet rows are the authoritative source ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    #   package_number  ΓåÆ display label for "Package #"
    #   pallet_number   ΓåÆ display label for "Pallet #"
    #   tracking_number ΓåÆ packages.tracking_number
    #   carrier         ΓåÆ packages.carrier_name
    ...
    # ΓöÇΓöÇ Claim financials ΓÇö claim_amount is a direct numeric column on claim_submissions ΓöÇΓöÇ
    ...
    ctx: dict[str, Any] = {
        ...
        "link_status":     _str(payload.get("link_status"), fallback="ΓÇö"),
        "notes":           _safe_text(
        ...
        # Photo URL slots ΓÇö populated below in priority order
        "outer_box_photo_url":      "",
        "box_label_photo_url":      "",
    ...
    # Priority 2: returns.photo_evidence ΓåÆ item photos
    if full_ret:
...
def _render_claim_evidence_pdf(
    ...
    * **Page 1** ΓÇö Master summary: batch header, intro paragraph, and a
      line-item table (ITEM NAME / ORDER ID / DEFECT TYPE / STATUS / PHOTO /
      DETAIL).
    * **Page 2** ΓÇö Detail view: IDENTIFIERS, LOGISTICS, CLAIM, NOTES /
    ...
    # ΓöÇΓöÇ Paragraph styles ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    _ss = getSampleStyleSheet()
    ...
    # ΓöÇΓöÇ Card grid helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    def _card(title: str, rows: list[tuple[str, str]]) -> Table:
        ...
        outer.setStyle(TableStyle([
            ...
            # kv body row ΓÇö zero extra padding (kv table handles its own)
    ...
    # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ PAGE 1 ΓòÉΓòÉ
    story.append(Paragraph("Claim evidence package", sty_pkg_title))
    story.append(Paragraph("BATCH SELECTION \u00b7 1 line item", sty_pkg_sub))
    story.append(
        Paragraph(
    ...
    # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ PAGE 2 ΓòÉΓòÉ
    story.append(Paragraph("Claim evidence package", sty_pkg_title))
    story.append(
        Paragraph(
    ...
    # ΓöÇΓöÇ 3-column card grid: IDENTIFIERS | LOGISTICS | CLAIM ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ NOTES / COMMENTS ΓÇö full-width styled card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Photographic evidence ΓÇö 3-column grid with bordered cells ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    story.append(Paragraph("Photographic evidence", sty_photo_hdr))

    PHOTO_COLS   = 3
    PHOTO_PAD    = 0.15 * cm                        # L+R cell padding
    photo_col_w  = bw / PHOTO_COLS
    photo_inner  = photo_col_w - 2 * PHOTO_PAD      # inner card width ΓÇö no overflow
...
def _upload_pdf_to_storage(
    ...
    Never raises ΓÇö failures are logged and returned as ``None`` values.
    """
    client = supabase_client
    if client is None:
        url = os.getenv("SUPABASE_URL") or os.getenv("NEXT_PUBLIC_SUPABASE_URL")
        key = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
        if not (url and key):
            _log("Storage upload: missing Supabase credentials ΓÇö skipping.")
    ...
    try:
        ...
        _log(f"PDF uploaded ΓåÆ {storage_bucket}/{storage_path}")
    except Exception as exc:
...
# ===========================================================================
# ΓöÇΓöÇ PUBLIC ENTRY POINT ΓöÇΓöÇ
# ===========================================================================
...
def generate_claim_evidence_pdf(
    *,
    ...
    It can be called from any workflow ΓÇö ``claim_agent``, a FastAPI endpoint,
    a CLI script, or a background job ΓÇö with no browser-automation dependency.
    ...
    dict::

        {
            ...
            "storage_path":   str | None,   # e.g. "reports/claim-evidence-ΓÇªpdf"
    ...
    """
    _log(
        f"generate_claim_evidence_pdf ΓÇö org={organization_id!r} "
    ...
    try:
        ...
        if claim_data is not None:
            ...
            _log(f"Loading claim context from DB for submission {submission_id!r}ΓÇª")
        ...
        if upload_to_storage:
            _log(f"Uploading to bucket {storage_bucket!r} ΓåÆ {organization_id}/{effective_sub_id}/ΓÇª")
