...
**OPERATOR-ACTION (live source sync — re-confirmed BLOCKED `20260620T080000Z`, first recorded `20260620T020500Z`):** `PHASE-AMAZON-INITIAL-LIVE-SOURCE-SYNC-EXECUTE-V1` is built + gated but BLOCKED. To run the initial live SP-API/Reports/Finances source sync (no claims created): (1) edit `.cursor/operator-approvals/amazon-initial-live-source-sync-v1-approval.md` → set `APPROVED_AMAZON_INITIAL_LIVE_SOURCE_SYNC_V1=yes`; (2) set the **12** missing env/worker flags in the LIVE deployment env (`ENABLE_AMAZON_REPORTS_API_WORKER` + the 8 per-source `ENABLE_AMAZON_REPORTS_API_*` + `ENABLE_AMAZON_FINANCES_API_WORKER`/`INGEST` + `CRON_SECRET`); (3) re-run `npx tsx scripts/phase-amazon-initial-live-source-sync-execute-v1.ts --execute`. Credentials already present (2 active marketplace rows). This also closes the 3-SKU sale-net data-coverage gap (settlement Order rows still 0/0/0 for I6-VR35-FSXQ / WD-VY8Z-CZ3F / 2H-7ZAX-Z2IP).
