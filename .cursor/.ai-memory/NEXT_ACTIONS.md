...
**DONE / RESUME (live source sync — EXECUTED `20260622T220000Z`):** `PHASE-AMAZON-INITIAL-LIVE-SOURCE-SYNC-EXECUTE-V1` ran for real — gates passed, **settlement imported 20,072 rows**, and the other 8 sources have resumable `source_run_id`s with reports generating on Amazon's side. **To finish the in-progress sources** (so removal/reimbursement/ledger/fee/returns/finances rows actually land): re-run `node --conditions=react-server --import tsx scripts/phase-amazon-initial-live-source-sync-run-v1.ts --execute` (idempotent within the UTC day — replays settlement, resumes the rest), and/or let the per-source resume routes / cron continue downloading once each report finishes generating; set `SYNC_MAX_ATTEMPTS=4+` to poll-and-import within a single run. NOTE on the still-open 3-SKU sale-net gap: a 30-day settlement window did NOT surface Order rows for `I6-VR35-FSXQ`/`WD-VY8Z-CZ3F`/`2H-7ZAX-Z2IP` (still 0/0/0) — widen `SYNC_WINDOW_DAYS` or run `PHASE-CLAIM-MISSING-SALE-PRICE-SOURCE-IMPORT-V1` against the older settlement period. If the Vercel deployment will run sync/cron, the same 12 keys + `CRON_SECRET` must be set there too.
