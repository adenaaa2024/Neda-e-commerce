...
**Last updated:** 2026-06-13 (`phase-claim-family-algorithm-matrix-v2-gap-expansion` `20260613T030000Z`)  
...
| `v_inventory_item_status` clean/disputed SQL gating V1 | **ORIGINAL APPLIED** `20260613T030022Z` — Maysam approved; migration `20260612120000_v_inventory_item_status_clean_disputed_gating_v1.sql` on `kxsvedvpjldygtdbylsy`; `387003587`/`X004LKS4VD`: **53→52** clean + **1** disputed; EP rows unchanged; `SAFE_ORIGINAL_VIEW_FIXED: yes`; evidence `phase-inventory-status-view-clean-disputed-gating-v1-original-apply/20260613T030022Z/` |
...
| **Claim source import + API plan V1** | **PLANNED** `20260613T002003Z` — 21 sources classified; 16 file-upload candidates; 4 API sync (removal live, reimbursements+settlements next); SAFE-T empty; stranded no table; SellerSnap COGS importer missing; `SAFE_TO_IMPLEMENT_SOURCE_IMPORT_UI: yes`; `SAFE_TO_IMPLEMENT_API_SYNC_NEXT: yes`; evidence `phase-amazon-claim-source-import-and-api-plan-v1/20260613T002003Z/` |
...
| **Claim family algorithm matrix V2 gap expansion** | **LOCKED** `20260613T030000Z` — 10 Maysam zip gaps evaluated; **34** full families (27 base + 7 gap entries); **5** claim-when-source-available + **5** review-signal-only; split `stranded_expired_review_signal` umbrella → stranded + expired signals; first safe new family `low_inventory_fee_issue`; `SAFE_TO_IMPLEMENT_V2_READMODEL: yes`; evidence `phase-claim-family-algorithm-matrix-v2-gap-expansion/20260613T030000Z/` |
...
| **Amazon fee/reimbursement estimate model V1** | **LOCKED** `20260613T014104Z` — 5 money lanes; 7 fee components; Product Fees API > Fee Preview > snapshot fallback; 4 reimbursement estimate variants; PC04 dims for fee recompute; product_cost_snapshots planned; staging fee_preview/storage empty; ORBIT unit_sale_price fallback flagged; `SAFE_TO_IMPLEMENT_FEE_ESTIMATE_READMODEL: yes_with_conditions`; evidence `phase-amazon-fee-and-reimbursement-estimate-model-v1/20260613T014104Z/` |
