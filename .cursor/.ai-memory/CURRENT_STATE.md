...
| **Claim evidence packet preview V1** | **IMPLEMENTED + VERIFIED** `20260615T091500Z` — `lib/claims/evidence/claim-evidence-packet-v1.ts`; API `GET /api/claims/center/evidence-packet` (`store_id`, `candidate_id`, `intake_run_id`, `limit`); read-only compose for original pilot **50/50** @ `a8a892fe` (30 `removal_shipment_missing` + 20 `removal_order_discrepancy`); ready_for_case_creation **50/50**; warnings only (`missing_fee`, `missing_cost`, `missing_photo_evidence`); no DB/candidate/case mutation; scanner untouched; build+smoke PASS; `SAFE_EVIDENCE_PACKET_PREVIEW_READY: yes`; `SAFE_TO_BUILD_EVIDENCE_PACKET_UI: yes`; evidence `phase-claim-evidence-packet-preview-v1/20260615T091500Z/` |
| **Claim evidence packet UI V1** | **IMPLEMENTED + VERIFIED** `20260615T100500Z` — pilot detail drawer evidence packet section (`ClaimPilotReviewEvidencePacketSection`); loads `GET /api/claims/center/evidence-packet` per candidate; readiness badges (Ready for case planning / Needs evidence review / Blocked); loading/error/empty states; disabled actions unchanged (Approve/Reject/Create case/Build PDF/Submit); original pilot **50/50** visible; sample verified shipment + order discrepancy; no DB mutation; build+smoke PASS; `SAFE_TO_REVIEW_EVIDENCE_PACKET_UI: yes`; `SAFE_TO_PLAN_CASE_CREATION_CONTRACT: yes`; evidence `phase-claim-evidence-packet-ui-v1/20260615T100500Z/` |
