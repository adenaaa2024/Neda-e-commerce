...
## Append — May 2026 (architecture correction)

| Risk | Detail |
|------|--------|
| **~5,333 bulk/orphan `return_items`** | expected_item_id only; no package/pallet — inflates scanned counts and could poison claims queue |
| **Returns claims work queue without physical-anchor gate** | Can surface orphan RIs with claimable conditions — **UNSAFE** |
| **Inventory views over-count scanned** | Orphan RIs counted in `v_scanned_items_counted` until quarantine or view filter |
| **Wave2 linkage copy (reverted)** | Rollback PASS — resolved RI restored to **57**; orphan rows remain |
| Merge/deploy before remediation | **Blocked** — no merge/original apply until quarantine + claims gate |
