...
export function ClaimCenterWorkflowBar() {
  ...
  return (
    <div
      className="claim-center-workflow-bar hidden lg:block sticky top-0 z-20 -mx-4 border-b border-black/5 bg-inherit/95 px-4 py-3 backdrop-blur-sm dark:border-white/10 sm:-mx-6 sm:px-6 xl:-mx-8 xl:px-8"
      data-claim-center="workflow-bar"
      role="navigation"
      aria-label="Claim recovery workflow"
    >
      <div className="flex items-center gap-2">
        <div className="claim-center-workflow-bar__track flex min-w-0 flex-1 items-stretch gap-1 overflow-x-auto overscroll-x-contain pb-0.5 [-webkit-overflow-scrolling:touch]">
          {CLAIM_CENTER_FLOW_STEPS.map((step, index) => {
            ...
            const isLast = index === CLAIM_CENTER_FLOW_STEPS.length - 1;

            return (
              <div key={step.id} className="claim-center-workflow-bar__segment flex shrink-0 items-center">
                <Link
                  href={step.href}
                  title={step.tooltip}
                  className={`claim-center-workflow-bar__step group flex min-h-[44px] min-w-[4.5rem] flex-col items-center justify-center rounded-xl px-2 py-1.5 text-center transition-all duration-150 sm:min-w-[5.5rem] sm:px-3 ${
                    active
                      ? "claim-center-workflow-bar__step--active"
                      : step.outline
                        ? "opacity-75 hover:opacity-100"
                        : "opacity-85 hover:opacity-100 hover:-translate-y-px"
                  }`}
                  aria-current={active ? "step" : undefined}
                >
                  <Icon className="h-4 w-4 shrink-0 opacity-80" aria-hidden />
                  <span className="mt-0.5 text-[10px] font-bold leading-tight sm:text-[11px]">{step.shortLabel}</span>
                  {showBadge ? (
                ...
                {!isLast ? (
                  <span className="claim-center-workflow-bar__connector mx-0.5 hidden h-px w-3 shrink-0 sm:block" aria-hidden />
                ) : null}
              </div>
            );
          })}
...
/** Compact step pill for mobile detail surfaces. */
