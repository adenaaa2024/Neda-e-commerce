...
import { CloudDownload, Loader2, Play, RefreshCw } from "lucide-react";
    ...
    <div className="rounded-xl border border-border bg-muted/30 px-4 py-3 text-sm">
      ...
      {snapshot.needs_resume ? (
        <p className="mt-3 text-xs text-amber-700 dark:text-amber-300">
          Amazon or the import pipeline may need another resume invocation. Use Resume or wait for
          auto-poll.
        </p>
      ) : null}
    </div>
...
export function ReportsApiReimbursementsPanel({ organizationId, onUploadComplete }: Props) {
  ...
  const flagsEnabled = flags?.reimbursements_enabled === true;
  ...
  const fetchRunSnapshot = useCallback(
    async (uploadId: string, apiNeedsResume?: boolean) => {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error ?? "Could not load source run status.");
      }
      const snap =
        json.source_run ??
  ...
  const callResume = useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/settings/imports/reports-api/resume", {
        method: "POST",
      ...
      if (res.status === 503) {
        throw new Error(json.error ?? "Reports API worker is disabled.");
      }
  ...
  const handlePull = useCallback(async () => {
    ...
    clearPoll();
    try {
      const res = await fetch("/api/settings/imports/reports-api/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <section className="rounded-2xl border border-sky-500/30 bg-card p-6 shadow-sm">
      <div className="flex items-center gap-2">
        <CloudDownload className="h-5 w-5 text-sky-500" aria-hidden />
        <h2 className="text-lg font-semibold text-foreground">Amazon Reports API — Reimbursements</h2>
      </div>
      <p className="mt-1 text-xs text-muted-foreground">
        Pull <span className="font-mono">GET_FBA_REIMBURSEMENTS_DATA</span> when server feature flags
        are enabled. Does not expose credentials.
      </p>
      ...
      {flags && !flagsEnabled && (
        <div className="mt-4 rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-100">
          <p className="font-medium">Reports API pull is disabled on this server.</p>
          <p className="mt-1 text-xs opacity-90">
            {flags.disabled_reason === "worker_disabled"
              ? "Set ENABLE_AMAZON_REPORTS_API_WORKER=true on the server."
              : "Set ENABLE_AMAZON_REPORTS_API_REIMBURSEMENTS=true (and master worker flag) on the server."}
          </p>
        </div>
      )}
      ...
      <div className={`mt-5 space-y-4 ${!flagsEnabled ? "pointer-events-none opacity-50" : ""}`}>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-lg bg-sky-600 px-4 text-sm font-medium text-white hover:bg-sky-700 disabled:opacity-50"
          >
            {busy === "pull" ? (
            ...
            Pull reimbursements from Amazon
          </button>
