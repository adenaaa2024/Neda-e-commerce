...
import {
  createClaimFilingRequest,
} from "@/lib/claim-filing-request-handlers";
import { isClaimFilingHandoffApiActive } from "@/lib/claim-filing-handoff";
import { supabaseServer } from "@/lib/supabase-server";

/**
 * POST /api/claims/filing-requests
 * Stub: creates `claim_filing_requests` + audit event. No external portal calls or workers.
 */
export async function POST(req: Request) {
  ...
  const result = await createClaimFilingRequest({
