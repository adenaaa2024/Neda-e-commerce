...
import { getCenterReadyToFilePayload } from "@/lib/claims/center/claim-center-api-handlers";
import {
...
export async function GET(req: Request) {
  ...
  if (!storeId) {
    return NextResponse.json(
      { error: "store_id is required for the ready-to-file queue." },
      { status: 400 },
    );
  }
  ...
  return runCenterGet(req, async ({ organizationId, storeId: gatedStoreId, url: u }) => {
    ...
    return getCenterReadyToFilePayload({
      organizationId,
