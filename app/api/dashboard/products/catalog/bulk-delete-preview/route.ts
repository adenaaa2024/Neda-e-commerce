...
/** POST bulk delete preview: dependency counts per product (no deletes). */
export async function POST(req: Request) {
  ...
  const { data, error } = await supabaseServer.rpc("pim_catalog_product_bulk_delete_preview", {
  ...
  if (error) {
    const msg = error.message ?? "Preview failed.";
    const m = msg.toLowerCase();
    const looksMissing =
      m.includes("pim_catalog_product_bulk_delete_preview") &&
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Delete preview RPC is not available. Apply migration 20260719140000_pim_products_soft_delete_and_delete_preview.sql.",
          details: msg,
        },
        { status: 503 },
      );
  ...
  return NextResponse.json(data && typeof data === "object" ? data : { ok: false, error: "Empty preview." });
}
