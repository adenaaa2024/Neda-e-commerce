...
} from "../../../../../../lib/pim-enrich-catalog-price-attempts";
import { pickAmazonApiPriceCandidateIndexWithOpenAI } from "../../../../../../lib/pim-price-candidate-ai";
import { pickAmazonCategoryLabelWithOpenAI } from "../../../../../../lib/pim-category-candidate-ai";
import {
...
} from "../../../../../../lib/pim-catalog-item-enrichment-fields";
import type { PricingApiTier } from "../../../../../../lib/pim-amazon-item-offers";
import {
  pickBestCatalogProductFallbackOfferPrice,
  pickBestCatalogProductListingPrice,
} from "../../../../../../lib/pim-saved-listing-price";
import {
...
} from "../../../../../../lib/pim-amazon-catalog-search";
...
} from "../../../../../../lib/pim-product-prices-insert";
...
type FailureRow = { product_id: string; reason: string };
...
export async function POST(req: Request) {
  let body: {
    ...
    product_ids?: string[];
    /** When true and caller is an org admin, response includes `enrichment_debug` rows. */
  ...
  if (retryMissingPrices && retryOnly) {
    return NextResponse.json(
      { ok: false, error: "Use either retry_missing_prices_only or retry_failed_only, not both." },
      { status: 400 },
    );
  }
  ...
  if (retryOnly && retryIds.length === 0) {
    return NextResponse.json(
      { ok: false, error: "retry_failed_only requires product_ids from the last run." },
      { status: 400 },
    );
  }

  if (retryMissingPrices && retryIds.length > 0) {
    return NextResponse.json(
      { ok: false, error: "retry_missing_prices_only does not use product_ids; omit that field." },
      { status: 400 },
    );
  }
  ...
  const allowEnrichmentDebug =
    Boolean(body.include_enrichment_debug) && (await userCanViewPimEnrichmentDebug(organizationId));
  ...
  if (!ctx.ok) {
    return NextResponse.json({ ok: false, error: ctx.error }, { status: 422 });
  }

  const tokenRes = await getAmazonCatalogAccessToken({ credentials: ctx.credentials });
  if (!tokenRes.ok) {
    return NextResponse.json(
      { ok: false, error: `Amazon authentication failed: ${tokenRes.error}` },
      { status: 502 },
    );
  }
  ...
  if (retryMissingPrices) {
    const { data: ppRows, error: ppQErr } = await supabaseServer
      ...
      .eq("store_id", storeId);
    if (ppQErr) {
      return NextResponse.json({ ok: false, error: ppQErr.message }, { status: 400 });
    }
