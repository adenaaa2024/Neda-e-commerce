...
import { runFbaReturnsReportsWorker } from "@/lib/amazon/reports-api-fba-returns-worker";
import { reportsApiDisabledReasonForFbaReturns } from "@/lib/amazon/reports-api-worker-flags";
...
export async function POST(req: Request): Promise<Response> {
  const disabled = reportsApiDisabledReasonForFbaReturns();
  if (disabled) {
    return NextResponse.json(
      {
        ...
        error:
          disabled === "worker_disabled"
            ...
            : "GET_FBA_FULFILLMENT_CUSTOMER_RETURNS_DATA is disabled (ENABLE_AMAZON_REPORTS_API_FBA_RETURNS).",
        code: disabled,
      },
      { status: 503 },
    );
  ...
  const result = await runFbaReturnsReportsWorker({
    organizationId,
    storeId,
  ...
  void auditPlatformAutomationManualRun({
    ...
    route: "/api/settings/imports/reports-api/fba-returns/run",
  });
