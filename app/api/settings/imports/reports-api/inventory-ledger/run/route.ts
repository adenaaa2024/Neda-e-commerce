...
import { runInventoryLedgerReportsWorker } from "@/lib/amazon/reports-api-inventory-ledger-worker";
import { reportsApiDisabledReasonForInventoryLedger } from "@/lib/amazon/reports-api-worker-flags";
...
export async function POST(req: Request): Promise<Response> {
  const disabled = reportsApiDisabledReasonForInventoryLedger();
  if (disabled) {
    return NextResponse.json(
      {
        ...
        error:
          disabled === "worker_disabled"
            ...
            : "GET_LEDGER_DETAIL_VIEW_DATA is disabled (ENABLE_AMAZON_REPORTS_API_INVENTORY_LEDGER).",
        code: disabled,
      },
      { status: 503 },
    );
  ...
  const result = await runInventoryLedgerReportsWorker({
    organizationId,
    storeId,
  ...
  void auditPlatformAutomationManualRun({
    organizationId,
    storeId,
    automationType: "inventory_ledger",
    action: "run_now",
    ...
    route: "/api/settings/imports/reports-api/inventory-ledger/run",
  });
