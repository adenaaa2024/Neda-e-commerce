...
import { runFeePreviewReportsWorker } from "@/lib/amazon/reports-api-fee-preview-worker";
import { reportsApiDisabledReasonForFeePreview } from "@/lib/amazon/reports-api-worker-flags";
...
export async function POST(req: Request): Promise<Response> {
  const disabled = reportsApiDisabledReasonForFeePreview();
  if (disabled) {
    return NextResponse.json(
      {
        ...
        error:
          disabled === "worker_disabled"
            ...
            : "GET_FBA_ESTIMATED_FBA_FEES_TXT_DATA is disabled (ENABLE_AMAZON_REPORTS_API_FEE_PREVIEW).",
        code: disabled,
      },
      { status: 503 },
    );
  ...
  const result = await runFeePreviewReportsWorker({
    organizationId,
    storeId,
  ...
  void auditPlatformAutomationManualRun({
    ...
    route: "/api/settings/imports/reports-api/fee-preview/run",
  });
