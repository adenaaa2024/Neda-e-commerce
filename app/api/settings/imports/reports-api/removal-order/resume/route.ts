...
export async function POST(req: Request): Promise<Response> {
  const disabled = reportsApiDisabledReasonForRemovalOrder();
  if (disabled) {
    return NextResponse.json(
      { ok: false, error: "Reports API removal order worker is disabled.", code: disabled },
      { status: 503 },
    );
  }
  ...
  const result = await runRemovalOrderReportsWorker(
    {
      organizationId,
  ...
  void auditPlatformAutomationManualRun({
    ...
    route: "/api/settings/imports/reports-api/removal-order/resume",
  });
